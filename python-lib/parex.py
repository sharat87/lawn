            ...
            for fd, state in active:
                ...
                if len(d) == 0:
                    ...
                    if len(remaining) == 1:
                        ...
                        self.trigger(Events.PROCESS_DONE, pid)
                else:
                    res = self.data.setdefault(pid, StringIO())
                    res.write(d)

    def trigger(self, event, pid):
        for handler in self.handlers[event]:
            handler(pid, self.data.get(pid, None))

    def on_process_done(self, handler):
        self.handlers.setdefault(Events.PROCESS_DONE, [])
        self.handlers[Events.PROCESS_DONE].append(handler)
