...
function! s:DrawSessionMenu()

	" Unset the menu
	silent! aunmenu File.Sessions

	" Fetch session files
	let files = substitute(glob(s:sessions_path . '/*'), '\\', '/', 'g')
	let files = substitute(files, "\\(^\\|\n\\)" . s:sessions_path . '/', '\1', 'g')
	let files = substitute(files, "\n[^\n]\\+x\\.vim\n", '\n', 'g')

	let sessions = split(files, '\n')

	if len(sessions) > 0
		for sfile in sessions
			" Extract filename
			exec "amenu 10.370 &File.S&essions.".escape(sfile,". ")." :SessionOpen ".escape(sfile,". ")."<CR>"
		endfor
		amenu &File.S&essions.-SessionsSep- <Nop>
	endif

	amenu &File.S&essions.&List             :SessionList<CR>
	amenu &File.S&essions.Open\ &Last       :SessionOpenLast<CR>
	amenu &File.S&essions.&Close            :SessionClose<CR>
	amenu &File.S&essions.&Save             :SessionSave<CR>
	amenu &File.S&essions.Save\ &As         :SessionSaveAs<CR>
endfunction
